...
# by CHASE_LINKS. For example, suppose /foo/bar is a link to the directory
# /alt/rod. Without this option set, 'cd /foo/bar/..' changes to /foo; with it
...
# described in the documentation for the cd builtin in Chapter 17 [Shell Builtin Commands],
# page 108. If the option is set, the shell does not test for directories beneath the local
...
# after changing to it are modified. It is no longer restricted to interactive shells
# (although printing of the directory stack with pushd is still limited to interactive
...
# If unset, key functions that list completions try to return to the last prompt if given
# a numeric argument. If set these functions try to return to the last prompt if given
# no numeric argument.

# ALWAYS_TO_END
# If a completion is performed with the cursor within a word, and a full completion is
# inserted, the cursor is moved to the end of the word. That is, the cursor is moved
# to the end of the word if either a single match is inserted or menu completion is
# performed.
...
# pletion, for example by pressing the tab key repeatedly. This option is overridden
# by MENU_COMPLETE.
...
# come directly after the name (like '}', ':', etc.), the automatically added character is
# deleted, so that the character typed comes immediately after the parameter name.
# Completion in a brace expansion is affected similarly: the added character is a ',',
...
# BASH_AUTO_LIST
# On an ambiguous completion, automatically list choices when the completion func-
# tion is called twice in succession. This takes precedence over AUTO_LIST. The setting
...
# pattern matching, not globbing, so it works not only for files but for any completion,
# such as options, user names, etc.
# Note that when the pattern matcher is used, matching control (for example, case-
# insensitive or anchored matching) cannot be used. This limitation only applies when
...
# beep if the option BEEP is also set; this may be modified if completion is called from
...
# BARE_GLOB_QUAL <Z>
# In a glob pattern, treat a trailing set of parentheses as a qualifier list, if it contains
# no '|', '(' or (if special) '~' characters. See Section 14.8 [Filename Generation],
# page 57.

# BRACE_CCL
# Expand expressions in braces which would not otherwise undergo brace expansion
# to a lexically ordered list of all the characters. See Section 14.6 [Brace Expansion],
# page 54.
...
# the result has more than one word the parameter will become an array with those
...
# or '=') performed on expression as if it were a parameter assignment. The argu-
# ment is not otherwise treated specially; it is passed to the command as a single
...
# except in sh emulation; otherwise it is off by default and has no effect if turned on.
...
# is designed purely for examining strings known to contain raw bytes or other values
# that may not be characters in the current locale. It is not necessary to unset the
# option merely because the character set for the current locale does not contain
...
# the consequence that references to words in history lines that have been read from a
# file may be inaccurate. When this option is set, words read in from a history file are
...
# unset, we instead truncate the old history file and write out the new version in-
# place. If one of the history-appending options is enabled, this option only has an
# effect when the enlarged history file needs to be re-written to trim it down to size.
# Disable this only if you have special needs, as doing so makes it possible to lose
...
# also possible to create a zle widget that will make some commands ignore imported
...
# a 'jobs' command, since it is assumed the user is aware that there are background
...
# that job control is no longer active. When the option is set, the MONITOR option and
# job control remain active in the subshell, but note that the subshell has no access
# to jobs in the parent shell.
...
# at the end of the partial line: a "%" for a normal user or a "#" for root. If set, the
# shell parameter PROMPT_EOL_MARK can be used to customize how the end of partial
# lines are shown.
...
# the command for which the DEBUG trap is being executed is skipped. The option is
...
# and the line number output by the prompt escape %i. If the option is set, the prompt
# escape %N will output the string '(eval)' instead of the script or function name as
...
# that are not restored are PRIVILEGED and RESTRICTED. Otherwise, only this option
# and the XTRACE and PRINT_EXIT_VALUE options are restored. Hence if this is ex-
...
# this option must be set prior to altering the trap behaviour in a function; unlike
# LOCAL_OPTIONS, the value on exit from the function is irrelevant. However, it does
# not need to be set before any global trap for that to be correctly restored by a
...
# BASH_REMATCH array will contain the entire matched text and subsequent elements
# will contain extracted substrings. This option makes more sense when KSH_ARRAYS
...
# Reserved words are described in Section 6.5 [Reserved Words], page 14.
...
# command builtin. Special builtins are ., :, break, continue, declare, eval, exit,
# export, integer, local, readonly, return, set, shift, source, times, trap and
# unset.
...
# characters in the local character set may be used in identifiers. Note that scripts
# and functions written with this feature are not portable, and also that both options
# must be set before the script or function is parsed; setting them during execution is
# not sufficient as the syntax variable=value has already been parsed as a command
...
# shell and output to files where necessary, although owing to restrictions of the library
# interface the string is truncated at the null character in file names, environment
...
# This option disables sourcing user startup files. If zsh is invoked as 'sh' or 'ksh'
# with this option set, /etc/suid_profile is sourced (after /etc/profile on inter-
# active shells). Sourcing ~/.profile is disabled and the contents of the ENV variable
...
# specified. If SHIN_STDIN is set explicitly on the command line, any argument that
# would otherwise have been taken as a file to run will instead be treated as a normal
# positional parameter. Note that setting or unsetting this option on the command
# line does not necessarily affect the state the option will have while the shell is running
...
# characters, assume that the zero-width characters will be displayed as modifications
# to the base character within the same width. Not all terminals handle this. If
# this option is not set, zero-width characters are displayed separately with special
# mark-up.
...
# editor. As it has no effect on shell syntax, many users may wish to disable this
